...
class Builder {
 public:
  ...
  invertible_cdf::plex::cht::CompactHistTree<KeyType> Finalize(
      size_t num_bins, size_t max_error) {
    ...
    auto lg = ComputeLog(static_cast<std::uint64_t>(max_key_ - min_key_), true);
 ...
 private:
  ...
  static unsigned ComputeLog(std::uint64_t n, bool round = false) {
    assert(n);
  ...
  static size_t GetNumShiftBits(std::uint64_t diff, size_t num_radix_bits) {
    uint32_t clzl = __builtin_clzl(diff);
      ...
      for (unsigned index = curr.first; index != curr.second; ++index) {
        ...
        if ((!currBin.has_value()) || (bin != currBin.value())) {
          ...
          for (unsigned iter = currBin.has_value() ? (currBin.value() + 1) : 0;
               iter != bin && iter < tree_[nodeIndex].second.size(); ++iter) {
  ...
  void TransformIntoRadixTable() {
    ...
    num_shift_bits_ = GetNumShiftBits(
        static_cast<std::uint64_t>(max_key_ - min_key_), num_radix_bits_);
    const uint32_t max_prefix = (max_key_ - min_key_) >> num_shift_bits_;
    table_.resize(max_prefix + 2, 0);
    for (size_t index = 0, limit = std::min(num_bins_, table_.size());
         index != limit; ++index) {

...
namespace dataset {
/// Unique IDs for each recognized dataset
enum class ID {
  SEQUENTIAL = 0,
  GAPPED_10 = 1,
  UNIFORM = 2,
...
template <class T>
T read_little_endian(const std::vector<unsigned char> &buf, size_t start) {
  T res = 0;
  for (size_t i = 0; i < sizeof(T); i++) {
    res |= static_cast<uint64_t>(buf[start + i]) << (i * sizeof(T));
  }
  return res;
}
...
std::vector<Key> load(const std::string &filepath) {
  ...
  std::vector<Key> dataset(max_num_elements, 0);
  {
    ...
    // Parse file
    uint64_t num_elements = read_little_endian<std::uint64_t>(buffer, 0);
    assert(num_elements <= max_num_elements);

    for (decltype(num_elements) i = 0; i < num_elements; i++) {
      // 8 byte header
      dataset[i] = read_little_endian<std::uint64_t>(
          buffer, i * sizeof(Key) + sizeof(decltype(num_elements)));
    }
  }
...
std::vector<Data> load_cached(ID id, size_t dataset_size) {
  ...
  switch (id) {
    ...
    case ID::FB: {
      if (ds_fb.empty()) {
        ds_fb = _::load<Data>("data/fb_200M_uint64");
    ...
    case ID::OSM: {
      if (ds_osm.empty()) {
        ds_osm = _::load<Data>("data/osm_cellids_200M_uint64");
    ...
    case ID::WIKI: {
      if (ds_wiki.empty()) {
        ds_wiki = _::load<Data>("data/wiki_ts_200M_uint64");
    ...
    case ID::BOOKS: {
      if (ds_books.empty()) {
        ds_books = _::load<Data>("data/books_200M_uint64");
  ...
  // sort before caching to avoid additional work in the future
